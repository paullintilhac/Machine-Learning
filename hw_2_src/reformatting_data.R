...
#then:
dat = read.csv("~/Machine Learning/hw_2_src/spambase.csv",header = F)
library(data.table)
dat = data.table(dat)
responses = dat[[58]]
dat[[58]]=NULL
...
#}
write.table(dat,
            file = "~/Machine Learning/hw_2_src/spambase.csv",
            row.names=F,
            col.names=F,
            sep = ",",
            quote = F)
...
write.table(training,
            file = "~/Machine Learning/hw_2_src/training.csv",
            row.names=F,
            col.names=F,
            sep=",",
            quote = F)
...
write.table(testing,
            file = "~/Machine Learning/hw_2_src/testing.csv",
            row.names=F,
            col.names =F,
            sep=",",
            quote = F)
